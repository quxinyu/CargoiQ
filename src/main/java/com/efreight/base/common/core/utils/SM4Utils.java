...
public class SM4Utils {
    ...
    public static String ecb(String secretKey, String context) {
        SM4Utils sm4 = new SM4Utils();
        sm4.secretKey = secretKey;
        return sm4.encryptData_ECB(context);
    }

    public static String dEcb(String secretKey, String context) {
        SM4Utils sm4 = new SM4Utils();
        sm4.secretKey = secretKey;
        return sm4.decryptData_ECB(context);
    }
    ...
    public String encryptData_ECB(String plainText) {
        try {
            ...
            byte[] encrypted = sm4_crypt_ecb(ctx, plainText.getBytes("UTF-8"));
    ...
    public String decryptData_ECB(String cipherText) {
        try {
            ...
            byte[] decrypted = sm4_crypt_ecb(ctx, decoder.decode(cipherText));
    ...
    public String encryptData_CBC(String plainText) {
        try {
            ...
            sm4_setkey_enc(ctx, keyBytes);
            byte[] encrypted = sm4_crypt_cbc(ctx, ivBytes, plainText.getBytes("UTF-8"));
            Base64.Encoder encoder = Base64.getEncoder();
            String cipherText = encoder.encodeToString(encrypted);
            if (cipherText != null && cipherText.trim().length() > 0) {
                Pattern p = Pattern.compile("\\s*|\t|\r|\n");
    ...
    public String decryptData_CBC(String cipherText) {
        try {
            ...
            sm4_setkey_dec(ctx, keyBytes);
            Base64.Decoder decoder = Base64.getDecoder();
            byte[] decrypted = sm4_crypt_cbc(ctx, ivBytes, decoder.decode(cipherText));
            return new String(decrypted, "UTF-8");
        } catch (Exception e) {
            e.printStackTrace();
            return null;
